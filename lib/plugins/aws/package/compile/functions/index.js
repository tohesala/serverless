...
class AwsCompileFunctions {
  ...
  compileFunction(functionName) {
    return BbPromise.try(() => {
      ...
      if (shouldVersionFunction || functionObject.provisionedConcurrency || functionObject.alias) {
        ...
        versionCompilationPromise = addFileContentsToHashes(artifactFilePath, [
          ...
          .then(() => {
            ...
            if (!functionObject.provisionedConcurrency && !functionObject.alias) return;
            ...
            const aliasResource = {
              ...
              Properties: {
                ...
                Name: functionObject.alias,
              },
              DependsOn: functionLogicalId,
            };

            let aliasLogicalId = this.provider.naming.getLambdaAliasLogicalId(functionName);

            if (functionObject.provisionedConcurrency) {
              const provisionedConcurrency = Number(functionObject.provisionedConcurrency);
              aliasResource.Properties.Name = aliasResource.Properties.Name || this.provider.naming.getLambdaProvisionedConcurrencyAliasName();
              aliasResource.Properties.ProvisionedConcurrencyConfig = {
                ProvisionedConcurrentExecutions: provisionedConcurrency,
              };

              aliasLogicalId = this.provider.naming.getLambdaProvisionedConcurrencyAliasLogicalId(functionName);
            }
            
            // use distinct logical id for provisioned concurrency alias to ensure backwards compatibility
            functionObject.targetAlias = { name: aliasResource.Properties.Name, logicalId: aliasLogicalId };
